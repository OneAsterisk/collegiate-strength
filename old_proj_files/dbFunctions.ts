...
// import { weightUnit } from '@/lib/stores/weightUnitStore';
  ...
  if (user) {
    ...
    try {
      ...
      const dotsScore: number = ageCoefCalc(
        age,
        Calculate_DOTS(bodyWeight, total, gender)
      );
    ...
    const promises = usersSnapshot.docs.map(async (userDoc) => {
      ...
      try {
        ...
        const universityLifts = liftsSnapshot.docs
          ...
          .filter((lift) =>
            universitiesMatch(lift.selectedUniversity, university)
          );
    ...
    (querySnapshot) => {
      const lifts: Lift[] = querySnapshot.docs.map((doc) => {
        ...
        const normalizedLift = {
          ...
          selectedUniversity:
            data.selectedUniversity || data.university || "Not Specified",
          ...
          total:
            data.total ||
            (data.squat || 0) + (data.bench || 0) + (data.deadlift || 0),
